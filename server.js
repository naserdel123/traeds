...
app.post('/api/signup', upload.single('avatar'), (req, res) => {
    ...
    if (db.users.find(u => u.username === username || u.email === email)) {
        return res.status(400).json({ error: 'الاسم أو البريد مستخدم مسبقاً' });
    }
...
app.post('/api/recover', (req, res) => {
    ...
    if (user) {
        res.json({ success: true, message: `تم العثور على الحساب! كلمة المرور هي: ${user.password}` });
    } else {
        res.status(400).json({ error: 'البريد الإلكتروني غير مسجل لدينا' });
    }
});
...
app.post('/api/posts', upload.fields([{ name: 'itemImage' }, { name: 'wantedImage' }]), (req, res) => {
    ...
    const newPost = {
        id: Date.now(), username, itemName, wantedName, tiktokLink,
...
app.get('/api/posts', (req, res) => res.json(getDB().posts.reverse()));
